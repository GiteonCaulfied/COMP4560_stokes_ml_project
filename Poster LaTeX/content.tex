...
\begin{itemize}
     \item In the field of computational geophysics, the continuum equations that govern a given physical phenomena (e.g. fluid flow inside the Earth's Mantle) is typically known. However, due to scarceness or uncertainty of data measurements, these equations cannot generate accurate predictions with data fed out-of-the-box.

     \item To solve this problem, the concept of inverse problem is proposed where the objective is to find a model using gradient-optimization techniques which best fits to the available data coming from partial, incomplete or noisy measurements. Unfortunately, the standard approach to solve the inverse problem, adjoint method, is also very computationally demanding, as it requires to solve a time-dependent nonlinear forward problem and an adjoint problem at each gradient-optimization step.

     \item The motivation of this research is, therefore, to reduce computational demands by evaluating the suitability of deep learning techniques as surrogate models that enable one to approximate the forward and adjoint problems with much less computational demands.

     \item As a first step towards this goal, we have tackled the forward problem of two classic problems in geodynamics, namely the Geoid and 2D Mantle convection problems.
\end{itemize}
...
\begin{itemize}
     \item This research aims to use Neural Network as a way to forward model the geoid problem, assuming that viscosity varying in the radial direction with depth is the most important factor. 
     
     \item The Earths viscosity is modelled as a 1D spherically symmetric viscosity model and a geoid surface is computed in terms of spherical harmonics coefficients, which can be used to construct the Geoid surface.  
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Sample Geoid coefficients, where the units of the geoid are in metres and represent values greater or less than some reference value.}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Animation sheet when FNN is either not used (ground truth at the first row) or used in S consecutive time steps before correction, where S = 1, 2, 4, 8, 16, 99 (started from the second row). The time steps, from left to right, are 1, 25, 50, 75, 100.}
\end{figure}
}
...
\begin{itemize}
     \item This research is presented as a proof showing that high-dimensional regression algorithms like neural networks can help with the forward modelling of the geoid problem when predicting a set of spherical harmonic coefficients given a reduced dataset with a fairly small number of 1D spherically symmetric viscosity models. 
\end{itemize}

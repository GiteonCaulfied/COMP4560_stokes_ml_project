...
\begin{figure}[H]
    ...
    \caption{Animation sheet when FNN is either not used (ground truth at the first row) or used in S consecutive time steps before correction, where S = 1, 2, 4, 8, 16, 99 (started from the second row). The time steps, from left to right, are 1, 25, 50, 75, 100. (see Fig 4 for colorbar)}
\end{figure}
}
